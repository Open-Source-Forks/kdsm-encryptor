// Contributors: Ensure that the environment variables are set correctly in your .env.local file.

import { Client, Account } from 'node-appwrite';

export const config = {
  project_id: process.env.APPWRITE_PROJECT,
  endpoint: process.env.APPWRITE_ENDPOINT,
  api_key: process.env.APPWRITE_API_KEY,
  database: process.env.APPWRITE_DATABASE,
};
export const collections = {
    api_keys: process.env.APPWRITE_COLLECTION_API_KEY,
    api_usage: process.env.APPWRITE_COLLECTION_API_USAGE,
    chatRooms: process.env.APPWRITE_COLLECTION_CHAT_ROOMS,
    share_encrypted_messages: process.env.APPWRITE_COLLECTION_SHARE_ENCRYPTED_MESSAGES,
    savedPasswords: process.env.APPWRITE_COLLECTION_SAVED_PASSWORDS,
    cryptoPassDeliveryConfig: process.env.APPWRITE_COLLECTION_CRYPTO_PASS_DELIVERY_CONFIG,
    securityQuestions: process.env.APPWRITE_COLLECTION_SECURITY_QUESTIONS,
    users: process.env.APPWRITE_COLLECTION_USERS,
}

/**
 * Get the logged-in user from session
 * @param {Request} request - Next.js request object
 * @returns {Promise<Object|null>} User object or null if not authenticated
 */
export async function getUserFromSession(request) {
  try {
    const sessionToken = request.cookies.get('kdsm-session')?.value;
    if (!sessionToken) return null;

    const client = new Client()
      .setEndpoint(config.endpoint)
      .setProject(config.project_id)
      .setSession(sessionToken);

    const account = new Account(client);
    return await account.get();
  } catch (error) {
    return null;
  }
}

/**
 * Create a session-based Appwrite client for the logged-in user
 * Use this when you need to perform account operations (like MFA)
 * @param {Request} request - Next.js request object
 * @returns {Promise<{account: Account, client: Client}>}
 */
export async function createSessionClient(request) {
  const sessionToken = request.cookies.get('kdsm-session')?.value;

  if (!sessionToken) {
    throw new Error('No session found. Please login.');
  }

  const client = new Client()
    .setEndpoint(config.endpoint)
    .setProject(config.project_id)
    .setSession(sessionToken);

  const account = new Account(client);

  return { account, client };
}

/**
 * Create an admin Appwrite client with API key
 * @returns {{account: Account, client: Client}}
 */
export function createAdminClient() {
  const client = new Client()
    .setEndpoint(config.endpoint)
    .setProject(config.project_id)
    .setKey(config.api_key);

  const account = new Account(client);

  return { account, client };
}